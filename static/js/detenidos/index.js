...
function listarDatos(){
    $.ajax({
        ...
        success: function(response){
            if($.fn.DataTable.isDataTable('#detenidos1')){
                $('#detenidos1').DataTable().destroy();
                
            }
            if($.fn.DataTable.isDataTable('#detenidos2')){
                $('#detenidos2').DataTable().destroy();
               
            }
            console.log(response);
        $('#detenidos1 tbody').html("");
        for(let i=0; i<response.length; i++){
            ...
            fila += '<td>' + response[i]["fields"]['nombre_detenido'] + '</td>';
            ...
            fila+='<button class="btn btn-primary" id= "modalButton" onclick="abrir_view_modal(\'/show-detenidos/'+ response[i]['pk']+'/\')">';
            ...
            fila+='<button class="btn btn-danger" id= "modalButton" onclick="delete_modal(\'/delete-detenidos/'+ response[i]['pk']+'/\')">';
            fila +=' <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">';
            ...
            fila+='</td>';
            fila += '</tr>';
            $('#detenidos1 tbody').append(fila);
        }

        $('#detenidos2 tbody').html("");
        for(let i=0; i<response.length; i++){
            ...
            fila += ' <path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/>';
            fila += ' </svg> ABRIR</button>';
            fila+='<button class="btn btn-success" id= "modalButton" onclick="activate_modal(\'/activate-detenidos/'+ response[i]['pk']+'/\')">';
            fila +=' <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">';
            ...
            fila+='</td>';
            fila += '</tr>';
            $('#detenidos2 tbody').append(fila);
        }
        
        $('#detenidos1').DataTable({
        ...
        */
        ...
        $('#detenidos1').DataTable().columns(1)
        .search('^ACTIVO$', true, false)
        .draw();

        $('#detenidos2').DataTable().columns(1)
