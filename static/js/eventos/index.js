...
function listarDatos(){
    $.ajax({
        ...
        success: function(response){
            if($.fn.DataTable.isDataTable('#eventos_table')){
                $('#eventos_table').DataTable().destroy();
                console.log("TABLA 1 DESTRUIDA")
            }
            if($.fn.DataTable.isDataTable('#eventos_table2')){
                $('#eventos_table2').DataTable().destroy();
                console.log("TABLA 2 DESTRUIDA")
            }
            console.log(response);
        $('#eventos_table tbody').html("");
        for(let i=0; i<response.length; i++){
            ...
            fila+='<button class="btn btn-primary" id= "modalButton" onclick="abrir_view_modal(\'/show-eventos/'+ response[i]['pk']+'/\')">';
            ...
            fila += ' </svg></button>';
            fila+='<button class="btn btn-warning" id= "modalButton" onclick="abrir_modal_edit(\'/update-eventos/'+ response[i]['pk']+'/\')">';
            ...
            fila += ' </svg></button>';
            fila+='<button class="btn btn-danger" id= "modalButton" onclick="delete_modal(\'/delete-eventos/'+ response[i]['pk']+'/\')">';
            fila +=' <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">';
            ...
            fila+='</td>';
            fila += '</tr>';
            $('#eventos_table tbody').append(fila);
        }

        $('#eventos_table2 tbody').html("");
        for(let i=0; i<response.length; i++){
            let fila = '<tr>';
            fila += '<td>' + (i+1) + '</td>';   
            fila += '<td>' + response[i]["fields"]['estatus'] + '</td>';
            ...
            fila += ' <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>';
            fila += ' </svg></button>';
            fila+='<button class="btn btn-success" id= "modalButton" onclick="activate_modal(\'/activate-eventos/'+ response[i]['pk']+'/\')">';
            fila +=' <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">';
            ...
            fila+='</td>';
            fila += '</tr>';
            $('#eventos_table2 tbody').append(fila);
        }
        ...
        */
        ...
        $('#eventos_table').DataTable().columns(1)
        .search('^ACTIVO$', true, false)
        .draw();

        $('#eventos_table2').DataTable().columns(1)
